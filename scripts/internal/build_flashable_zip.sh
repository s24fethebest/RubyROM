...
FILE_NAME="RubyROM_${ROM_STATUS}_${ROM_VERSION}_$(date +%Y%m%d)_${TARGET_CODENAME}${ZIP_FILE_SUFFIX}"
while [ -f "$OUT_DIR/$FILE_NAME" ]; do
    INCREMENTAL=$((INCREMENTAL + 1))
    FILE_NAME="RubyROM_${ROM_VERSION}_$(date +%Y%m%d)-${INCREMENTAL}_${TARGET_CODENAME}${ZIP_FILE_SUFFIX}"
done
...
PRINT_HEADER()
{
    ...
    echo    'ui_print("****************************************************");'
    echo -n 'ui_print("'
    echo -n "Welcome to RubyROM $ROM_CODENAME $ROM_VERSION for $TARGET_NAME!"
    echo    '");'
    echo    'ui_print("RubyROM developed by AhmetTheDev0725(s24fethebest@GitHub)");'
    ...
    echo    '");'
    echo    'ui_print("****************************************************");'
    echo    'ui_print("Thank you for installing and using RubyROM! <3");'
    echo    '");'
    echo    'ui_print("We wish that you have the best experience while");'
    echo    'ui_print("using RubyROM");'
    echo    'ui_print("****************************************************");'
